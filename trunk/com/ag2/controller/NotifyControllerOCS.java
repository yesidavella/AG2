...
public class NotifyControllerOCS implements NotificableOCS, Serializable {
    ...
    @Override
    public void notifyJobSentCreatedOCS(final Entity entitySource, final Entity entityDestination, final long jobsSent) {
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (!MatchCoupleObjectContainer.containsIntanceOCS(graphNodeSource, graphNodeDestination)) {
                    ...
                    if (directionCreated) {
                        MatchCoupleObjectContainer.getGraphOCS(graphNodeDestination, graphNodeSource).setOCSJobSentInverted(jobsSent);
                    }                    
                    
                    
                } else {
                    MatchCoupleObjectContainer.getGraphOCS(graphNodeSource, graphNodeDestination).setOCSJobSent(jobsSent);
                }
